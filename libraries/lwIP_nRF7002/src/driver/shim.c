...
static unsigned int shim_qspi_read_reg32(void *priv, unsigned long addr)
{
	unsigned int val;
	struct shim_bus_qspi_priv *qspi_priv = priv;
	struct qspi_dev *dev;
...
static void shim_qspi_write_reg32(void *priv, unsigned long addr, unsigned int val)
{
	struct shim_bus_qspi_priv *qspi_priv = priv;
	struct qspi_dev *dev;
...
static void shim_qspi_cpy_from(void *priv, void *dest, unsigned long addr, size_t count)
{
	struct shim_bus_qspi_priv *qspi_priv = priv;
	struct qspi_dev *dev;
...
static void shim_qspi_cpy_to(void *priv, unsigned long addr, const void *src, size_t count)
{
	struct shim_bus_qspi_priv *qspi_priv = priv;
	struct qspi_dev *dev;
...
static void *shim_nbuf_alloc(unsigned int size)
{
	...
	nwb = (struct nwb *)calloc(sizeof(struct nwb), sizeof(char));

	if (!nwb)
		return NULL;

	nwb->priv = calloc(size, sizeof(char));
...
void *net_pkt_to_nbuf(struct net_pkt *pkt)
{
	...
	nwb = shim_nbuf_alloc(len + 100);

	if (!nwb) {
		return NULL;
	}

	shim_nbuf_headroom_res(nwb, 100);

	data = shim_nbuf_data_put(nwb, len);
...
void *net_pkt_from_nbuf(void *iface, void *frm)
{
	...
	len = shim_nbuf_data_size(nwb);

	data = shim_nbuf_data_get(nwb);
...
static void *shim_llist_node_alloc(void)
{
	...
	llist_node = calloc(sizeof(*llist_node), sizeof(char));
...
{
	struct shim_llist_node *zep_llist_node = NULL;

	zep_llist_node = (struct shim_llist_node *)llist_node;
...
static void *shim_llist_alloc(void)
{
	...
	llist = calloc(sizeof(*llist), sizeof(char));
...
{
	struct shim_llist *zep_llist = NULL;

	zep_llist = (struct shim_llist *)llist;
...
{
	struct shim_llist *zep_llist = NULL;
	struct shim_llist_node *zep_node = NULL;

	zep_llist = (struct shim_llist *)llist;
	zep_node = (struct shim_llist_node *)llist_node;
...
{
	struct shim_llist_node *zep_head_node = NULL;
	struct shim_llist *zep_llist = NULL;
	...
	zep_head_node = (struct shim_llist_node *)sys_dlist_peek_head(&zep_llist->head);
...
{
	struct shim_llist_node *zep_node = NULL;
	struct shim_llist_node *zep_nxt_node = NULL;
	struct shim_llist *zep_llist = NULL;

	zep_llist = (struct shim_llist *)llist;
	zep_node = (struct shim_llist_node *)llist_node;

	zep_nxt_node = (struct shim_llist_node *)sys_dlist_peek_next(&zep_llist->head,
									 &zep_node->head);
...
static void *shim_work_alloc(void)
{
	return work_alloc();
}

static void shim_work_free(void *item)
{
	return work_free(item);
}

static void shim_work_init(void *item, void (*callback)(unsigned long data),
				  unsigned long data)
{
	work_init(item, callback, data);
}

static void shim_work_schedule(void *item)
{
	work_schedule(item);
}

static void shim_work_kill(void *item)
{
	work_kill(item);
}
...
static enum wifi_nrf_status shim_bus_qspi_dev_init(void *os_qspi_dev_ctx)
{
...
static void shim_bus_qspi_dev_deinit(void *os_qspi_dev_ctx)
{
	struct qspi_dev *dev = NULL;
...
static void *shim_bus_qspi_dev_add(void *os_qspi_priv, void *osal_qspi_dev_ctx)
{
	struct shim_bus_qspi_priv *zep_qspi_priv = NULL;
...
static void shim_bus_qspi_dev_rem(void *os_qspi_dev_ctx)
{
	struct qspi_dev *dev = NULL;
...
static void *shim_bus_qspi_init(void)
{
	struct shim_bus_qspi_priv *qspi_priv = NULL;

	qspi_priv = calloc(sizeof(*qspi_priv), sizeof(char));
...
static void shim_bus_qspi_deinit(void *os_qspi_priv)
{
	struct shim_bus_qspi_priv *qspi_priv = NULL;
...
static void shim_bus_qspi_dev_host_map_get(void *os_qspi_dev_ctx,
...
static void irq_work_handler(struct k_work *work)
{
	...
	intr_priv =
		(struct shim_intr_priv *)CONTAINER_OF(work, struct shim_intr_priv, work);
...
static void shim_irq_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	struct shim_intr_priv *intr_priv = NULL;

	intr_priv = (struct shim_intr_priv *)cb;

	k_work_schedule(&intr_priv->work, K_NO_WAIT);
}

static enum wifi_nrf_status shim_bus_qspi_intr_reg(void *os_dev_ctx, void *callbk_data,
						       int (*callbk_fn)(void *callbk_data))
{
	...
	intr_priv = calloc(sizeof(*intr_priv), sizeof(char));
	...
	ret = rpu_irq_config(&intr_priv->gpio_cb_data, shim_irq_handler);
...
static void shim_bus_qspi_intr_unreg(void *os_qspi_dev_ctx)
{
}
...
static void shim_timer_init(void *timer, void (*callback)(unsigned long), unsigned long data)
{
...
static const struct wifi_nrf_osal_ops wifi_nrf_os_zep_ops = {
	...
	.qspi_read_reg32 = shim_qspi_read_reg32,
	.qspi_write_reg32 = shim_qspi_write_reg32,
	.qspi_cpy_from = shim_qspi_cpy_from,
	.qspi_cpy_to = shim_qspi_cpy_to,
	...
	.tasklet_alloc = shim_work_alloc,
	.tasklet_free = shim_work_free,
	.tasklet_init = shim_work_init,
	.tasklet_schedule = shim_work_schedule,
	.tasklet_kill = shim_work_kill,
	...
	.bus_qspi_init = shim_bus_qspi_init,
	.bus_qspi_deinit = shim_bus_qspi_deinit,
	.bus_qspi_dev_add = shim_bus_qspi_dev_add,
	.bus_qspi_dev_rem = shim_bus_qspi_dev_rem,
	.bus_qspi_dev_init = shim_bus_qspi_dev_init,
	.bus_qspi_dev_deinit = shim_bus_qspi_dev_deinit,
	.bus_qspi_dev_intr_reg = shim_bus_qspi_intr_reg,
	.bus_qspi_dev_intr_unreg = shim_bus_qspi_intr_unreg,
	.bus_qspi_dev_host_map_get = shim_bus_qspi_dev_host_map_get,
