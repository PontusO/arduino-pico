...
netif *nRF7002::_netif = nullptr;

nRF7002::nRF7002(int8_t cs, arduino::SPIClass& spi, int8_t intrpin) {
    (void) cs;
    (void) spi;
    (void) intrpin;
    _netif = nullptr;
...
bool nRF7002::begin(const uint8_t* address, netif* netif) {
    (void) address;
    _netif = netif;
...
uint16_t nRF7002::sendFrame(const uint8_t* data, uint16_t datalen) {
#if 0
...
uint16_t nRF7002::readFrame(uint8_t* buffer, uint16_t bufsize) {
...
extern "C" void cyw43_cb_process_ethernet(void *cb_data, int itf, size_t len, const uint8_t *buf) {
    ...
    (void) cb_data;
    (void) itf;
    struct netif *netif = nRF7002::_netif; // &self->netif[itf];
#if CYW43_NETUTILS
    if (self->trace_flags) {
...
extern "C" void cyw43_cb_tcpip_set_link_up(cyw43_t *self, int itf) {
    ...
    (void) itf;
    if (nRF7002::_netif) {
//        netif_set_link_up(nRF7002::_netif);
    }
}
...
extern "C" void cyw43_cb_tcpip_set_link_down(cyw43_t *self, int itf) {
    ...
    (void) itf;
    if (nRF7002::_netif) {
//        netif_set_link_down(nRF7002::_netif);
    }
    self->wifi_join_state &= ~WIFI_JOIN_STATE_ACTIVE;
}

extern "C" int cyw43_tcpip_link_status(cyw43_t *self, int itf) {
    //if ((nRF7002::_netif->flags & (NETIF_FLAG_UP | NETIF_FLAG_LINK_UP)) == (NETIF_FLAG_UP | NETIF_FLAG_LINK_UP))
    //  Fake this since it's only used in the SDK
    if ((nRF7002::_netif->flags & (NETIF_FLAG_LINK_UP)) == (NETIF_FLAG_LINK_UP)) {
        return 0;
